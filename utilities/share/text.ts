...
export const SHARE_TEXTS = {
  MILESTONE_COMPLETED: (title: string, slugOrUid: string, grantUid: string) =>
    `Just hit a major milestone for my grant from ${title}!\nTrack the progress on ${SOCIALS.X_HANDLE} GAP → https://karmahq.xyz/project/${slugOrUid}/funding?grantId=${grantUid}&tab=milestones-and-updates\n\nWould love your thoughts and feedback!`,
  ...
  PROJECT_ACTIVITY: (title: string, slugOrUid: string) =>
    `📢 Project activity dropped for ${title}!\nIt’s not just talk—it’s trackable. Onchain, transparent, and live via ${SOCIALS.X_HANDLE} GAP →  https://karmahq.xyz/project/${slugOrUid}/updates\nLet me know what you think!`,
  ...
  MILESTONE_PENDING: (title: string, slugOrUid: string, grantUid: string) =>
    `Just posted a milestone for my grant from ${title}! Follow along with my progress on ${SOCIALS.X_HANDLE} GAP → https://karmahq.xyz/project/${slugOrUid}/funding?grantId=${grantUid}&tab=milestones-and-updates\n\nStay tuned for updates!`,
};
