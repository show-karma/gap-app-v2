...
describe('queryKeys', () => {
  ...
  describe('integration tests', () => {
    ...
    it('should have consistent naming patterns', () => {
      ...
      expect(QUERY_KEYS.GRANTS.DUPLICATE_CHECK({ community: 'a', title: 'b' })[0]).toContain('grant');
    });
    ...
    it('should always return tuples (arrays)', () => {
      ...
      expect(Array.isArray(QUERY_KEYS.CONTRACTS.VALIDATION.VALIDATE({ address: 'a', network: 'b' }))).toBe(true);
      expect(Array.isArray(QUERY_KEYS.COMMUNITY.PROJECT_UPDATES('a', 'b', 1))).toBe(true);
      expect(Array.isArray(QUERY_KEYS.GRANTS.DUPLICATE_CHECK({ community: 'a', title: 'b' }))).toBe(true);
    });
  });
});
