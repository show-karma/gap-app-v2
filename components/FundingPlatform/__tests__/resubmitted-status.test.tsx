...
describe("Resubmitted Status in Admin UI", () => {
	...
	describe("Filter Options", () => {
		it("should include resubmitted in status filter dropdown", () => {
			...
			const resubmittedOption = filterOptions.find(
				(opt) => opt.value === "resubmitted",
			);
		...
		it("should position resubmitted after pending in filter options", () => {
			...
			const pendingIndex = filterOptions.findIndex(
				(opt) => opt.value === "pending",
			);
			const resubmittedIndex = filterOptions.findIndex(
				(opt) => opt.value === "resubmitted",
			);
	...
	describe("Status Transitions", () => {
		...
		it("should have same transitions as pending for resubmitted", () => {
			...
			expect(STATUS_TRANSITIONS.resubmitted).toEqual(
		...
		it("should show Start Review button for resubmitted applications", () => {
			...
			> = {
				pending: [
					{ targetStatus: "under_review", label: "Start Review", variant: "primary" },
				],
				resubmitted: [
					{ targetStatus: "under_review", label: "Start Review", variant: "primary" },
				],
				...
				revision_requested: [
					{ targetStatus: "under_review", label: "Review", variant: "primary" },
				],
				approved: [],
				rejected: [],
			};
	...
	describe("Feature Requirements", () => {
		...
		it("should differentiate resubmitted from pending in statistics", () => {
			...
			const activeApplications =
				stats.pendingApplications + (stats.resubmittedApplications || 0);
