...
export const PostApprovalDataView: FC<PostApprovalDataViewProps> = ({ application, program }) => {
                ...
                <div className="space-y-2">
                  ...
                  {item.description && (
                    <div className="text-sm text-gray-600 dark:text-gray-400 prose prose-sm dark:prose-invert max-w-none">
                      <MarkdownPreview source={item.description} />
                    </div>
                  )}
                </div>
              </div>
  ...
  const dataToRender = application.postApprovalData;

  if (!dataToRender || Object.keys(dataToRender).length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 dark:text-gray-400">No post-approval data available</p>
      </div>
    );
  }
