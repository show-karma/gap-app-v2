...
const AdminCommunityHeader = ({ community }: { community: CommunityDetailsResponse }) => {
  return (
    <div
      className={cn(
        layoutTheme.padding,
      ...
      <div className="flex flex-row gap-4 flex-wrap max-lg:flex-col justify-between items-center w-full">
        <div className="flex h-max flex-1 flex-row items-center justify-start gap-3 ">
          <div className="flex justify-center bg-black rounded-full p-2">
            <Image
              alt={(community as CommunityDetailsResponse)?.details?.name || "Community"}
              src={(community as CommunityDetailsResponse)?.details?.logoUrl || "/placeholder.png"}
              width={24}
              height={24}
          ...
          <div className="flex flex-col gap-0">
            <p className="text-3xl font-body font-semibold text-black dark:text-white max-2xl:text-2xl max-lg:text-xl">
              {community ? (community as CommunityDetailsResponse)?.details?.name : ""}
            </p>
          </div>
        </div>
      </div>
...
const NormalCommunityHeader = ({ community }: { community: CommunityDetailsResponse }) => {
          ...
          >
            <div className="flex justify-center border border-white rounded-full p-2">
              <Image
                alt={(community as CommunityDetailsResponse)?.details?.name || "Community"}
                src={
                  (community as CommunityDetailsResponse)?.details?.logoUrl || "/placeholder.png"
                }
                width={56}
                height={56}
                className={
...
export default function CommunityHeader({ community }: { community: CommunityDetailsResponse }) {
