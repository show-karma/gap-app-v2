...
export const TabTrigger = ({ value, className, icon, children, disabled = false }: TabTriggerProps) => {
  ...
  return (
    ...
    >
      {children}
      {icon ? (
        <p className={cn("rounded-full px-2.5", isActive ? "text-[#155EEF] bg-[#F2F4F7]" : "text-[#667085]")}>
          {icon}
        </p>
      ) : null}
    </Button>
