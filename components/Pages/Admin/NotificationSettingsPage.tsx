...
function TelegramConfigCard({
  ...
  const { mutate: saveConfig, isPending: isSaving } = useSaveNotificationConfig(communityUID);
  const { mutate: deleteConfig, isPending: isDeleting } = useDeleteNotificationConfig(communityUID);
  const { mutate: testConfig, isPending: isTesting } = useTestNotificationConfig(communityUID);
  ...
  const handleDelete = () => {
    ...
    deleteConfig(config.id, {
      onSuccess: () => toast.success("Telegram configuration deleted"),
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-4">
        ...
        <div>
          ...
          {form.formState.errors.botToken && (
            <p className="text-sm text-red-600 mt-1">{form.formState.errors.botToken.message}</p>
          )}
        </div>
        ...
        <div>
          ...
          {form.formState.errors.chatId && (
            <p className="text-sm text-red-600 mt-1">{form.formState.errors.chatId.message}</p>
          )}
        </div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button
            ...
            variant="secondary"
            onClick={handleTest}
            disabled={isTesting || !form.watch("botToken") || !form.watch("chatId")}
          >
...
function SlackConfigCard({
  ...
  const onSubmit = (data: SlackFormData) => {
    saveConfig(
      ...
      {
        onSuccess: () => toast.success("Slack configuration saved"),
        onError: (err) => toast.error(err.message || "Failed to save"),
      }
    );
  };
  ...
  const handleDelete = () => {
    if (!config?.id) return;
    if (!confirm("Delete this Slack configuration?")) return;
    deleteConfig(config.id, {
      onSuccess: () => toast.success("Slack configuration deleted"),
      onError: (err) => toast.error(err.message || "Failed to delete"),
    });
  };
    ...
    <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-4">
        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-zinc-800 rounded-lg">
          ...
          />
          <label htmlFor="slack-enabled" className="text-sm font-medium text-gray-900 dark:text-white">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button
            ...
            variant="secondary"
            onClick={handleTest}
            disabled={isTesting || !form.watch("webhookUrl")}
          >
            {isTesting ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Send className="w-4 h-4 mr-2" />}
            Test
          </Button>
...
export function NotificationSettingsPage({ community }: NotificationSettingsPageProps) {
  ...
  const { configs, isLoading, error } = useNotificationConfigs(communitySlug);
  ...
  // Community-level configs only (programId is null)
  const communityConfigs = configs.filter((c) => !c.programId);
  const telegramConfig = communityConfigs.find((c) => c.providerType === "TELEGRAM");
  const slackConfig = communityConfigs.find((c) => c.providerType === "SLACK");
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <p className="text-sm text-blue-700 dark:text-blue-300">
          <strong>Note:</strong> These settings replace email for reviewer, admin, and finance notifications.
          Applicant and grantee emails are not affected.
        </p>
      </div>

      <div className="space-y-6">
        <TelegramConfigCard config={telegramConfig} communityUID={communitySlug} />
        <SlackConfigCard config={slackConfig} communityUID={communitySlug} />
      </div>
    </div>
  );
}
