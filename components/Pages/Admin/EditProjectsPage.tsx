                ...
                <tbody className="px-4 divide-y divide-gray-200 dark:divide-zinc-800">
                    {projects.map((project) => {
                        ...
                        const currentRegionName =
                            projectRegions.length > 0 ? projectRegions[0] : "";

                        // Find the region ID from the name
                        const currentRegionId = currentRegionName
                            ? regions.find((r) => r.name === currentRegionName)
                                  ?.id || ""
                            : "";
                        ...
                        const displayRegionName =
                            optimisticRegion !== undefined
                                ? regions.find((r) => r.id === optimisticRegion)
                                      ?.name || "None"
...
export default function EditProjectsPage() {
    ...
    const [selectedRegions, _setSelectedRegions] = useState<
    ...
    const { hasAccess, isLoading: isLoadingAdmin } = useCommunityAdminAccess(
        community?.uid,
    );
    ...
    const { grants } = useCommunityGrants(
