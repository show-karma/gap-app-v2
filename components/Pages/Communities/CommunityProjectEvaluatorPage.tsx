...
function ChatWithKarmaCoPilot({
  ...
  return (
    <div className="relative flex flex-col h-[500px] bg-white rounded-lg border shadow-sm mb-8">
      {/* Messages Container */}
      <div
        className="flex-1 overflow-y-auto p-4 space-y-4 community-evaluator-chat-container"
        data-chat-container
      >
      ...
      <div className="w-full flex flex-col gap-1">
        ...
        <div className="flex w-full flex-col px-3">
          ...
          <p className="mb-2 text-sm font-medium text-gray-400 dark:text-zinc-400 max-2xl:text-[13px]">
            Created on{" "}
            {new Date(
              project.projectDetails.data.createdAt
            ).toLocaleDateString()}
          </p>
...
export const CommunityProjectEvaluatorPage = () => {
  ...
  async function getProjectsByProgram(programId: string, communityId: string) {
    try {
      setIsLoadingProjects(true);
      const [projects, error] = await fetchData(
        INDEXER.PROJECTS.BY_PROGRAM(programId, communityId)
      );
      if (error) {
  ...
  useEffect(() => {
    if (selectedProgram) {
      setProjects([]);
      setIsLoading(true);
      getProjectsByProgram(selectedProgram.programId, communityId);
      setIsLoading(false);
    }
  ...
  const handleProgramSelect = (program: Program) => {
    ...
    getProjectsByProgram(program.programId, communityId); // Fetch projects for the selected program
  };
      ...
      {!selectedProgram ? (
        <div className="flex flex-col items-center justify-center h-full bg-gray-50 p-4">
          ...
          <div className="flex flex-wrap justify-center items-center gap-4 mb-8 w-full">
            <div className="bg-zinc-100 rounded-3xl p-4 text-lg text-gray-700 shadow-md">
              ✅ Compare projects within the program
            </div>
            <div className="bg-zinc-100 rounded-3xl p-4 text-lg text-gray-700 shadow-md">
              ✅ Analyze project metrics, updates, outcomes and impact
            </div>
          </div>
