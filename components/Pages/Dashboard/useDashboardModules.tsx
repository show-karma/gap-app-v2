...
import { SkeletonList } from "./v3/primitives";
 ...
 * kept out of the hook body to stay under Biome's complexity budget.
 */
...
export interface DashboardModulesState {
  /** The role modules to render as tiles / drill-ins (already gated). */
  modules: DashModule[];
  /** Whether auth/permissions/token are still resolving (hold a skeleton). */
  isLoading: boolean;
  authenticated: boolean;
  address: Hex | undefined;
  /** Show the super-admin panel (staff only). */
  showSuperAdmin: boolean;
  /** Permissions couldn't be verified — surface a warning banner. */
...
/**
 * The dashboard's data + role orchestration, independent of presentation.
 *
 * Runs all six role queries (gated on a resolved auth token to avoid the
 * Privy-flicker refetch storm), collapses each into a tile status, builds the
 * `DashModule[]`, and drives the logged-out → home redirect. Both the in-place
 * `<Dashboard />` and the route-based `DashboardProvider` consume this so the
 * gating logic lives in exactly one place.
 */
export function useDashboardModules(): DashboardModulesState {
