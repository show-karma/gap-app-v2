...
export const LinkGithubRepoButton: FC<
    LinkGithubRepoButtonProps
    ...
    const validateGithubRepo = async (repo: string, index: number) => {
        ...
        try {
            ...
            if (!repoUrl.hostname.includes('github.com')) {
                setValidationErrors(prev => ({ ...prev, [index]: 'Not a valid GitHub URL' }));
                setValidatedRepos(prev => ({ ...prev, [index]: false }));
                return false;
            }
        } catch (e) {
            setValidationErrors(prev => ({ ...prev, [index]: 'Invalid URL format' }));
            setValidatedRepos(prev => ({ ...prev, [index]: false }));
            return false;
        }
        ...
        if (pathParts.length < 2) {
            setValidationErrors(prev => ({ ...prev, [index]: 'Invalid GitHub repository URL' }));
            setValidatedRepos(prev => ({ ...prev, [index]: false }));
            return false;
        }
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    setValidationErrors(prev => ({ ...prev, [index]: 'Repository not found or private' }));
                } else {
                    setValidationErrors(prev => ({ ...prev, [index]: `GitHub API error: ${response.statusText}` }));
                }
                setValidatedRepos(prev => ({ ...prev, [index]: false }));
                return false;
            }
            ...
            if (repoData.private) {
                setValidationErrors(prev => ({ ...prev, [index]: 'Repository is private. Only public repositories are supported.' }));
                setValidatedRepos(prev => ({ ...prev, [index]: false }));
                return false;
            }
            ...
            setValidationErrors(prev => ({ ...prev, [index]: 'Failed to validate repository' }));
            setValidatedRepos(prev => ({ ...prev, [index]: false }));
            return false;
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (error) {
                ...
                toast.error("Failed to update GitHub repositories. Please try again.");
            }
        } catch (err) {
            setError(`Failed to update GitHub repositories. Please try again.`);
            console.error(err);
                                ...
                                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl dark:bg-zinc-800 bg-white p-6 text-left align-middle transition-all ease-in-out duration-300">
                                    ...
                                    >
                                        ...
                                        <p className="text-md text-gray-500 dark:text-gray-400 mt-2">
                                            Add one or more GitHub repository URLs for the project. Only public repositories are supported.
                                        </p>
                                    </Dialog.Title>
                                        ...
                                        {repos.map((repo, index) => (
                                            <div key={index} className="flex flex-col">
                                                    ...
                                                    <div className="flex items-center justify-between p-4 bg-gray-100 dark:bg-zinc-700 rounded-lg flex-grow">
                                                        ...
                                                        <div className="flex items-center">
                                                            {validatingRepo === index ? (
                                                                <span className="text-sm animate-pulse">Validating...</span>
                                                ...
                                                {validationErrors[index] && (
                                                    <p className="text-red-500 text-sm mt-1 ml-4">{validationErrors[index]}</p>
                                                )}
                                            </div>
                                        ))}
                                        <Button
                                    ...
                                    <div className="flex flex-row gap-4 mt-10 justify-end">
                                        <Button
                                            onClick={handleSave}
                                            disabled={isLoading || Object.keys(validationErrors).length > 0 || !areAllReposValidated()}
