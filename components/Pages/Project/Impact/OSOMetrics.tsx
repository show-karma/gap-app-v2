        ...
        return (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {Array.from({ length: 12 }).map((_, i) => (
                    <div key={i} className="h-[104px] w-full animate-pulse rounded-lg bg-gray-100" />
                ))}
            </div>
        );
    }
    ...
    const getCodeMetrics = (): MetricConfig[] => {
        ...
        if (codeMetrics.timeToFirstResponseDaysAverage6Months > 0) {
            metrics.push({
                ...
                value: `${codeMetrics.timeToFirstResponseDaysAverage6Months.toFixed(1)}d`,
    ...
    const getOnchainMetrics = (): MetricConfig[] => {
        ...
        if (onchainMetrics.multiProjectAddressCount90Days > 0) {
            metrics.push({
                ...
                tooltip: "Number of addresses active in multiple projects in the last 90 days"
            });
        }
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {codeMetricsList.map((metric, index) => (
                            <MetricCard
                                key={index}
                                title={metric.title}
                                value={metric.isFormatted ? metric.value : formatNumber(metric.value as number)}
                                icon={metric.icon}
                ...
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <div className="flex items-center gap-2 text-gray-800">
                        ...
                        <div>
                            ...
                            <p className="text-sm">No activity metrics found for this project</p>
                        </div>
                    </div>
                </div>
            )}
