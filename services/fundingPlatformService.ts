...
apiClient.interceptors.request.use((config) => {
  ...
  const token = typeof window !== 'undefined' ? localStorage.getItem('gap-auth-token') : null;
  if (token) {
...
export const fundingProgramsAPI = {
   ...
   */
  async getProgramsByCommunity(communityId: string): Promise<any[]> {
    const response = await apiClient.get(`/grant-programs/communities/${communityId}/grant-programs`);
   ...
   */
  async getProgramConfiguration(programId: string, chainId: number): Promise<IFundingProgramConfig> {
    const response = await apiClient.get(`/grant-programs/${programId}/${chainId}/configuration`);
  ...
  async updateProgramConfiguration(
    ...
    const backendConfig = {
      ...config,
      isEnabled: config.enabled, // Map enabled to isEnabled
      chainID: config.chainId || chainId, // Map chainId to chainID
    };
    
    // Remove frontend-specific fields that backend doesn't expect
    delete backendConfig.enabled;
    delete backendConfig.chainId;
    
    const response = await apiClient.put(`/grant-programs/${programId}/${chainId}/configuration`, backendConfig);
    const data = response.data;
  ...
  async updateFormSchema(
    ...
    const response = await apiClient.put(`/grant-programs/${programId}/${chainId}/configuration`, updatedConfig);
    return response.data;
  },
  ...
  async toggleProgramStatus(programId: string, chainId: number, enabled: boolean): Promise<IFundingProgramConfig> {
    const response = await apiClient.put(`/grant-programs/${programId}/${chainId}/toggle-status`, { isEnabled: enabled });
    const data = response.data;
  ...
  async getProgramStats(programId: string, chainId: number): Promise<any> {
    const response = await apiClient.get(`/grant-programs/${programId}/${chainId}/stats`);
    return response.data;
  },
...
export const fundingApplicationsAPI = {
  ...
  async submitApplication(
    ...
    const response = await apiClient.post(`/grant-programs/${programId}/${chainId}/applications`, {
      applicantEmail,
      applicationData,
    });
  ...
  async getApplications(
    ...
    const response = await apiClient.get(`/grant-programs/${programId}/${chainId}/applications?${params}`);
    const data = response.data;
  ...
  async getApplication(applicationId: string): Promise<IFundingApplication> {
    const response = await apiClient.get(`/grant-programs/applications/${applicationId}`);
  ...
  async updateApplicationStatus(
    ...
    const response = await apiClient.put(`/grant-programs/applications/${applicationId}/status`, {
      status,
      note,
    });
  ...
  async getApplicationStatistics(programId: string, chainId: number): Promise<IApplicationStatistics> {
    const response = await apiClient.get(`/grant-programs/${programId}/${chainId}/applications/statistics`);
    return response.data;
  },
  ...
  async exportApplications(
    ...
    const response = await apiClient.get(`/grant-programs/${programId}/${chainId}/applications/export?${params}`);
    return response.data;
  },
};
