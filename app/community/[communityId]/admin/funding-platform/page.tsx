...
import { PlusIcon, CogIcon, EyeIcon, ChevronDownIcon } from "@heroicons/react/24/solid";
...
export default function FundingPlatformAdminPage() {
  ...
  const statistics = useMemo(() => {
    ...
    const stats = programs.reduce((acc, program) => {
      ...
      const programStats = program.stats || program.grantPlatform?.stats || {};
      return {
        totalPrograms: acc.totalPrograms + 1,
        totalApplications: acc.totalApplications + (programStats.total || programStats.applicationCount || 0),
        approved: acc.approved + (programStats.approved || 0),
        rejected: acc.rejected + (programStats.rejected || 0),
        pending: acc.pending + (programStats.pending || programStats.submitted || 0)
      };
    }, {
      totalPrograms: 0,
      totalApplications: 0,
            ...
            >
              ...
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {program.deadline || program.endDate ? (
                      <>
                        Deadline is {new Date(program.deadline || program.endDate).toLocaleDateString()}{' '}
                        <span className="text-red-600 ml-1">01:12:13 min left</span>
                      </>
                    ) : (
                ...
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                  {program.title || program.metadata?.title || 'Web3 Analytics Infrastructure Grant'}
                </h3>
                
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  {program.description || program.metadata?.description || 'Retroactive funding for projects that have already delivered significant impact to the Ethereum and Optimism ecosystems.'}
                </p>

                {/* Grant Amount and Applicants */}
                <div className="flex justify-between text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Total Grant Amount</span>
                    <div className="font-medium text-gray-900 dark:text-white">
                      {program.totalAmount || program.metadata?.totalAmount || '+1M available'}
                    </div>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Total Applicants</span>
                    <div className="font-medium text-gray-900 dark:text-white">
                      {(program.stats?.total || program.stats?.applicationCount || program.grantPlatform?.stats?.total || 144)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {((program.stats?.pending || program.stats?.submitted || program.grantPlatform?.stats?.pending || 0) > 0) && (
                <div className="p-3 bg-orange-50 dark:bg-orange-900/20 border-b border-gray-200 dark:border-gray-700">
                  <Link href={`/community/${communityId}/admin/funding-platform/${program.programId}_${program.chainID}/applications`}>
                    <button className="w-full text-left flex items-center justify-between text-orange-700 dark:text-orange-300 text-sm hover:text-orange-800 dark:hover:text-orange-200">
                      <span>Review {program.stats?.pending || program.stats?.submitted || program.grantPlatform?.stats?.pending || 120} Pending Applications</span>
