        ...
        <div className="flex flex-row w-full max-w-full px-4 pb-4 sm:px-6 lg:px-8 py-5 border-b border-b-[#DFE1E6]">
            <div className="flex flex-row items-start gap-4 w-full">
                <div
                    ...
                    <Image height={48} width={48} src={community.details?.logoUrl} alt={community.details?.name} className="h-12 w-12 min-w-12 min-h-12 rounded-full border border-white" />
                </div>
                <div className="flex flex-col gap-0">
                    <div className="flex flex-row items-center gap-3">
                        <Link
                            href={PAGES.COMMUNITY.DONATE_PROGRAM(community.details.slug || community.uid, programId)}
                            className="text-sm font-medium text-blue-600 underline"
                        >
                            Program Donations
                        </Link>
                        <ChevronRightIcon className="w-4 h-4 text-neutral-300" />
                        <p className="text-sm font-normal text-black dark:text-white">Donation Checkout {items.length > 0 && `(${items.length} ${pluralize('item', items.length)})`}</p>
                    </div>
                    <h1 className="text-3xl font-bold text-black dark:text-white">{program.metadata?.title}</h1>
                    <ReadMore markdownClass="text-sm text-gray-800 dark:text-gray-200" side="left">{program.metadata?.description || ''}</ReadMore>
                </div>
            </div>
        </div>
    )
