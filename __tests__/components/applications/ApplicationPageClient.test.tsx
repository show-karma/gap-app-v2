...
const mockUseApplicationAccess = vi.fn(() => ({
  accessInfo: null,
  isLoading: false,
  error: null,
  refetch: vi.fn(),
...
vi.mock("@/src/features/applications/hooks/use-application-access", () => ({
  useApplicationAccess: (...args: unknown[]) => mockUseApplicationAccess(...args),
}));

vi.mock("@/src/features/applications/components/ApplicationStatusHistory", () => ({
...
describe("ApplicationPageClient", () => {
  ...
  it("shows edit button when wallet user is the owner", () => {
    mockUseApplicationAccess.mockReturnValue({
      accessInfo: null, isLoading: false, error: null, refetch: vi.fn(),
