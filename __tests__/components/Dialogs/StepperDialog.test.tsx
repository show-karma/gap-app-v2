...
describe("StepperDialog", () => {
  ...
  describe("Step Colors", () => {
    it("should use blue color for completed and active steps", () => {
      ...
      const blueSteps = Array.from(stepCircles).filter(
        (el) => (el as HTMLElement).style.backgroundColor === "rgb(76, 111, 255)"
      );
    ...
    it("should use gray color for pending steps", () => {
      ...
      const graySteps = Array.from(stepCircles).filter(
        (el) => (el as HTMLElement).style.backgroundColor === "rgb(83, 92, 104)"
      );
  ...
  describe("Step Progression", () => {
    ...
    it("should correctly identify step numbers", () => {
      ...
      expect(screen.getByText("Preparing to write to blockchain").className).toContain(
        "text-black"
      );
      expect(screen.getByText("Executing the onchain transaction").className).toContain(
        "text-black"
      );
