 ...
 *
 * `app/t/[tenant]/(chrome)/community/[communityId]/(cover)/` is a sibling route group to
...
async function coverLayout(children: ReactNode, communityId = "filecoin") {
  const { default: CoverLayout } = await import(
    "@/app/t/[tenant]/(chrome)/community/[communityId]/(cover)/layout"
  );
...
async function withHeaderLayout(children: ReactNode, communityId = "filecoin") {
  const { default: WithHeaderLayout } = await import(
    "@/app/t/[tenant]/(chrome)/community/[communityId]/(with-header)/layout"
  );
...
describe("(cover) layout renders no explorer chrome", () => {
  ...
  it("names none of the explorer chrome components in its code", () => {
    ...
    const source = readFileSync(
      path.join(REPO_ROOT, "app/t/[tenant]/(chrome)/community/[communityId]/(cover)/layout.tsx"),
      "utf8"
    )
      .replace(/\/\*[\s\S]*?\*\//g, "")
      .replace(/\/\/.*$/gm, "");
