...
import { ChevronRight, ChevronDown, CircleHelp, CircleUser, LogOutIcon, PhoneCall, ToggleLeft, ToggleRight, Wallet, FolderKanban, ShieldCheck, CheckCircle2, Settings, Heart  } from "lucide-react";
                        ...
                        <MenubarContent align="end" className="flex flex-col gap-4 px-4 py-4 w-max">
                            <div className="flex flex-col w-full">
                                <MenubarItem className="w-full hover:bg-transparent focus:bg-transparent text-muted-foreground cursor-pointer">
                                    <div className="flex flex-row items-center gap-2">
                                        {address ? <span className="text-sm text-muted-foreground font-medium hover:text-muted-foreground">{formatAddressLong(address)}</span> : <span className={menuStyles.itemText}>No wallet connected</span>}
                                    </div>
                                </MenubarItem>
                            </div>
                                    ...
                                    {socialMediaLinks.map((social) => {
                                        ...
                                        return (
                                            <ExternalLink
                                                key={social.name}
                                                href={social.href}
                                                className={cn(menuStyles.itemText, "flex items-center justify-center rounded-full transition-colors p-2")}
