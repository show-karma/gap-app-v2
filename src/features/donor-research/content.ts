/**
 * Single source of truth for the /nonprofit-research FAQ — rendered
 * visibly by `ResearchFaqSection` and emitted as FAQPage JSON-LD by the
 * page, so the structured data can never drift from the visible content
 * (the reviewed E1 pattern from /for-agents).
 *
 * Every claim traces to gap-indexer donor-research code or approved
 * shipped copy — sources noted per entry. The compliance wording is
...
export const RESEARCH_FAQS: ResearchFaqEntry[] = [
  ...
  // competitors' products.
  {
    ...
    answer:
      "Karma Nonprofit Research is built for the shortlist-and-recommend workflow rather than one-at-a-time lookups: instead of searching organizations individually, an advisor describes the donor's criteria once and receives a ranked, compliance-checked shortlist with the evidence for each check stated inline, ready to share with the donor. It draws on the same public record — IRS 990 and 990-PF filings — plus live activity signals.",
  },
  // Access. Source: sign-in gate + advisor-scoped services.
  {
    question: "Who can use Karma Nonprofit Research?",
    answer:
      "The research workspace is for donor advisors and requires signing in — reports, personas, and diligence requests are private to the advisor who created them. Donors access shared reports through a private link from their advisor without needing an account.",
  },
];
