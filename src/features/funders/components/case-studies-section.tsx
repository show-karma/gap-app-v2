...
const caseStudyCards: CaseStudyCardType[] = [
  ...
  },
  {
    ...
    description:
      "Over the past 10 months, Celo has leveraged Karma's platform to track 400+ projects, 850+ grants and 3600+ milestones, moving from fragmented tracking to a unified onchain project registry and funding funnel to simplify impact measurement and grow their ecosystem.",
    communitySlug: "celo",
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-0">
        ...
        <div className={cn("text-foreground font-normal text-sm", "leading-[150%] tracking-[0%]")}>
          {card.description}
        </div>
      </div>
    ...
    <div className={cn("flex flex-col justify-between", "min-h-[317px] h-full w-full", "p-5")}>
      <div className="flex flex-col justify-between h-full max-md:gap-2 gap-8">
        <div className="flex flex-col gap-8">
          ...
          <p className={cn("text-foreground font-normal text-sm", "leading-[150%] tracking-[0%]")}>
            {card.text}
          </p>
        </div>
        {(card.author || card.authorRole) && (
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              {card.author && (
                <span
                  className={cn("text-foreground font-bold text-sm", "leading-5 tracking-[0%]")}
                >
                  {card.author}
                </span>
              )}
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        {community && imageUrl ? (
          <div className="flex items-center gap-2">
            ...
            <span className={cn("text-foreground font-bold text-sm", "leading-5 tracking-[0%]")}>
              {community.name}
            </span>
          </div>
        ) : (
