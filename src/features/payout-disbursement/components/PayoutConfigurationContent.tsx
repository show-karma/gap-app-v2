...
>(function PayoutConfigurationContent(
  ...
  // Reset form when becoming inactive
  useEffect(() => {
    if (!isActive) {
      setPayoutAddress("");
      setTotalGrantAmount("");
  ...
  const isDirty = useMemo(() => {
    ...
    if (!currentConfig) {
      return payoutAddress !== "" || totalGrantAmount !== "";
    }
  ...
  useImperativeHandle(
    ...
    [handleSave, isDirty, isSaving] // eslint-disable-line react-hooks/exhaustive-deps
  );

  if (isLoadingConfig || isLoadingMilestones) {
    return (
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <select
            ...
            onChange={(e) => setSelectedNetwork(Number(e.target.value) as SupportedChainId)}
        ...
        <div>
          ...
          >
            ...
            <option value="native">{getNativeTokenSymbol(selectedNetwork)} (Native)</option>
          </select>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="group relative">
            ...
            <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-64 p-3 bg-gray-900 text-white text-xs rounded-lg shadow-lg z-10">
              <p>
                Allocate the total grant amount across payments. The sum must equal the total grant
                amount. Set amount to 0 for payments you want to skip.
              </p>
            </div>
          </div>
        </div>
        ...
        {milestones.length === 0 && (
          <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
            This grant has no milestones defined. Use First payment, Final payment, and optional
            custom line items.
          </p>
        )}
      </div>
