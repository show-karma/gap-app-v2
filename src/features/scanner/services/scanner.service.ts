...
// Karma `x-api-key`, both attached by the `api` client when `isAuthorized` is
// true and the base URL is the indexer. Calls therefore go straight to the
// indexer (browser and server alike), same as every other Karma endpoint.
//
...
//
// The `api` client throws a typed `HttpError` (with `.status`) on failure,
// so callers that used to read `error.status` off the wrapped Error keep
// working unchanged — the thrown ApiError already carries that field.
...
export async function submitScan(payload: SubmitScanRequest): Promise<SubmitScanResponse> {
  ...
  return api.post<SubmitScanResponse>(`${SCANNER_BASE}/scans`, payload, { isAuthorized: true });
}
...
export async function getScanByUrl(url: string): Promise<DetailScorecardPayload | null> {
  try {
    // TODO(#1775): add zod schema
    return await api.get<DetailScorecardPayload>(`${SCANNER_BASE}/scans`, {
      params: { url },
      isAuthorized: true,
    });
  } catch (e) {
...
export async function getScanById(scanId: string): Promise<DetailScorecardPayload> {
  // TODO(#1775): add zod schema
  return api.get<DetailScorecardPayload>(`${SCANNER_BASE}/scans/${scanId}`, {
    isAuthorized: true,
  });
}
...
export async function getPublicScorecardBySlug(slug: string): Promise<PublicScorecardPayload> {
  // TODO(#1775): add zod schema
  return api.get<PublicScorecardPayload>(`${SCANNER_BASE}/reports/${slug}`, {
    isAuthorized: false,
  });
}
...
export async function refreshScan(scanId: string): Promise<SubmitScanResponse> {
  // TODO(#1775): add zod schema
  return api.post<SubmitScanResponse>(
...
export async function submitContactRequest(payload: ContactRequest): Promise<{ id: string }> {
  // TODO(#1775): add zod schema
  return api.post<{ id: string }>(`${SCANNER_BASE}/contact`, payload, { isAuthorized: false });
}
