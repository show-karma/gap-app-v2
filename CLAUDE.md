...
- **Analytics**: Every new user-facing flow emits catalog events from `utilities/analytics/events.ts` via `track()` — never a raw event string, and never `mixpanel-browser` outside `utilities/analytics/client.ts`. A new event needs three things together: the catalog entry, a row in `docs/analytics/tracking-plan.md`, and a unit test asserting the name and props at the emit site. Never put an email, a wallet address or a raw pathname on an event — those belong on the profile (`identifyUser`) or in `route_pattern`. Enforced by `scripts/check-anti-patterns.sh` (`[ANALYTICS]`) and `__tests__/unit/utilities/analytics/tracking-plan.test.ts`.
